...
Circle from_three_points(Point p1, Point p2, Point p3)
{
    ...
    f = ((ax - cx) * (p1.x - p2.x) + (ay - cy) * (p1.x - p2.x) + (bx - ax) * (p1.x - p3.x) + (by - ay) * (p1.x - p3.x)) / (2 * ((p3.y - p1.y) * (p1.x - p2.x) - (p2.y - p1.y) * (p1.x - p3.x)));

    g = ((ax - cx) * (p1.y - p2.y) + (ay - cy) * (p1.y - p2.y) + (bx - ax) * (p1.y - p3.y) + (by - ay) * (p1.y - p3.y)) / (2 * ((p3.x - p1.x) * (p1.y - p2.y) - (p2.x - p1.x) * (p1.y - p3.y)));
